...
return {
  ...
  -- ── Snacks (dashboard, picker, explorer, toggles, …) ──────────────────
  {
    ...
    opts = {
      ...
      --   respect_gitignore = false  show .gitignored files too (optional)
      --
    ...
    keys = {
      ...
      -- FIX-HIDDEN-FILES (2026-06-26): <leader>ff now shows dotfiles.
      -- Previously: Snacks.picker.files() hid dotfiles by default.
      -- Now: pass { hidden = true, respects_gitignore = false } so
      -- .gitignore, .env, .stylua.toml etc. appear in find-files results.
