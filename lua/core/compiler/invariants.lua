...
function M.assert_stage_forward(from_stage, to_stage, context)
  ...
  for i, stage in ipairs(stage_order) do
    if stage == from_stage then from_idx = i end
    if stage == to_stage then to_idx = i end
  end

  if from_idx and to_idx and to_idx < from_idx then
    error(string.format("Invariant VIOLATION (INV-6): Stage transition backwards: %s -> %s in %s", from_stage, to_stage, context), 2)
  end
end
...
function M.assert_ir_shape(ir, context)
  ...
  if ir.stage == ir_mod.STAGES.LIR then
    if not ir.caps or not ir.resolved or not ir.merged_lsp or not ir.all_parsers then
      error(string.format("Invariant VIOLATION (INV-1): LIR missing required fields in %s", context), 2)
    end
  end
end
...
function M.check_phase_output(ir_in, ir_out, phase_name)
  ...
  if ir_in == ir_out then
    error(string.format("Invariant VIOLATION (INV-1): Phase '%s' returned the same IR table (must be COW)", phase_name), 2)
  end
end
...
function M.assert_strategy_shape(strategy, context)
  if not _enabled then return end
  if type(strategy) ~= "table" or not strategy.name or not strategy.resolve or not strategy.priority then
