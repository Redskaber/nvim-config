...
R.describe("core.domain.schema", function()
  ...
  R.it("validates empty cap table → ok", function() assert_ok({}) end)

  -- ── lsp ───────────────────────────────────────────────────────────────────

  R.describe("lsp", function()
    R.it(
      "accepts valid lsp entry",
      function() assert_ok({ lsp = { lua_ls = { settings = {} } } }) end
    )
    R.it(
      "rejects non-table lsp server config",
      function() assert_err({ lsp = { bad = "string" } }, "expected table") end
    )
    R.it(
      "rejects non-string lsp key",
      function() assert_err({ lsp = { [42] = {} } }, "non-empty string") end
    )
  end)
  ...
  R.describe("formatters", function()
    R.it(
      "accepts plain string formatter",
      function() assert_ok({ formatters = { lua = { "stylua" } } }) end
    )
    R.it(
      "accepts FormatterNode with strategy",
      function()
    ...
    R.it("rejects raw function formatter", function()
      assert_err(
        { formatters = {
          python = function() return {} end,
        } },
        "raw function"
      )
    end)
    R.it(
      "rejects sentinel string in formatter list",
      function() assert_err({ formatters = { lua = { "__STRATEGY__" } } }, "sentinel") end
    )
    ...
    R.it(
      "rejects non-string/non-table entry in formatter list",
      function() assert_err({ formatters = { lua = { 42 } } }, "expected string or FormatterNode") end
    )
  end)
  ...
  R.describe("linters", function()
    R.it(
      "accepts valid linter list",
      function() assert_ok({ linters = { python = { "ruff" } } }) end
    )
    R.it(
      "rejects non-string linter entry",
      function() assert_err({ linters = { python = { { kind = "formatter" } } } }, "linter entries") end
    )
    R.it(
      "rejects sentinel in linters",
      function() assert_err({ linters = { lua = { "__LINT__" } } }, "sentinel") end
    )
  end)
  ...
  R.describe("treesitter", function()
    R.it("accepts string list", function() assert_ok({ treesitter = { "lua", "python" } }) end)
    R.it(
      "rejects non-string entry",
      function() assert_err({ treesitter = { "lua", 42 } }, "expected string") end
    )
  end)
  ...
  R.describe("mason", function()
    R.it(
      "accepts non-empty string list",
      function() assert_ok({ mason = { "stylua", "ruff" } }) end
    )
  ...
  R.describe("version compatibility", function()
    R.it("version=1 accepted silently", function() assert_ok({ version = 1 }) end)
    R.it("version=0 (older) accepted silently", function() assert_ok({ version = 0 }) end)
  ...
  R.it(
    ...
    function() R.assert_eq(schema.format_diags({}), "") end
  )
