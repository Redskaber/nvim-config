...
R.describe("runtime.pipeline", function()
  ...
  R.describe("debug_run → collect stage", function()
    ...
    R.it("caps is deep-copy; modifying result doesn't affect next run", function()
      local ir1 = pipeline.debug_run({ "modules.lang.lua" }, "collect")
      ir1.caps.lua = nil
      local ir2 = pipeline.debug_run({ "modules.lang.lua" }, "collect")
      R.assert_not_nil(ir2.caps.lua, "registry must not be affected by IR mutation")
    end)
  end)
  ...
  R.describe("debug_run → canonicalize stage", function()
    R.it("IR.symbols has lsp and tools sub-tables", function()
      local ir = pipeline.debug_run({ "modules.lang.lua" }, "canonicalize")
  ...
  R.describe("debug_run → optimize stage", function()
    R.it("all_parsers is deduplicated", function()
      local ir = pipeline.debug_run({ "modules.lang.lua", "modules.lang.python" }, "optimize")
  ...
  R.describe("run()", function()
    R.it("returns non-empty specs for lua", function()
    ...
    R.it("stable spec count across two runs (determinism)", function()
      local s1 = pipeline.run({ "modules.lang.lua" }, "full")
      local s2 = pipeline.run({ "modules.lang.lua" }, "full")
      R.assert_eq(#s1, #s2)
    end)
    ...
    R.it("returns (specs, ir) two-value tuple", function()
      ...
      R.assert_true(#specs > 0)
      R.assert_not_nil(ir.caps and ir.caps.lua)
    end)
    ...
    R.it("accepts cached_caps and skips collect phase", function()
      ...
      local specs = pipeline.run({ "modules.lang.lua" }, "full", ir_ast.caps)
      R.assert_true(#specs > 0)
    end)
  ...
  R.describe("debug_run freeze flag", function()
    ...
    R.it("debug_run does not affect pipeline.state()", function()
      ...
      pipeline.debug_run({ "modules.lang.lua" }, "canonicalize")
  ...
  R.describe("ir.diff() integration", function()
    R.it("detects stage change between collect and optimize", function()
      local ir_ast = pipeline.debug_run({ "modules.lang.lua" }, "collect")
      local ir_lir = pipeline.debug_run({ "modules.lang.lua" }, "optimize")
  ...
  R.describe("PHASE_ORDER contract", function()
    ...
    R.it("first entry is 'collect'", function() R.assert_eq(pipeline.PHASE_ORDER[1], "collect") end)
    R.it(
      "last entry is 'codegen'",
      function() R.assert_eq(pipeline.PHASE_ORDER[#pipeline.PHASE_ORDER], "codegen") end
    )
  end)
end)